...
describe Citygram::Models::Subscription do
  ...
  it 'whitelists mass-assignable attributes' do
    expect(Subscription.allowed_columns.sort).to eq [:publisher_id, :contact, :channel, :geom].sort
  end
  ...
  describe 'contact channel validations' do
    context 'webhook' do
      it 'requires the contact to be a valid url' do
        subscription = build(:subscription, channel: 'webhook', contact: Faker::Internet.email)
        expect(subscription).not_to be_valid
      end
    end
    ...
    context 'sms' do
      it 'requires the contact to be a valid phone number', pending: 'best way to validate phone numbers?' do
        subscription = build(:subscription, channel: 'sms', contact: Faker::Internet.uri('https'))
        expect(subscription).not_to be_valid
      end
    end
    ...
    context 'email' do
      it 'requires the contact to be a valid email address' do
        subscription = build(:subscription, channel: 'email', contact: Faker::PhoneNumber.short_phone_number)
        expect(subscription).not_to be_valid
      end
    end
  end
